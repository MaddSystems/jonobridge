...
class vecfleetService(BaseService):
    @property
    def service_name(self) -> str:
        """Return the name of the service, which matches the database table name."""
        return "vecfleet"  # This must match the database table name
    ...
    def parameters_helpers(self) -> Dict[str, str]:
        ...
        return {
            "vecfleet_url": ["https://api.staging.vecfleet.io/auth"],   
            "vecfleet_post_url": ["https://api.staging.vecfleet.io/gps/avl"],  
            "vecfleet_email": ["scania-mx@vecfleet.io"],
            "vecfleet_name": ["Scania-mx"],
            "vecfleet_password": ["vdp<T7GK"],
            "plates_url": ["https://pluto.dudewhereismy.com.mx/imei/search?appId=1141"],
            "service": ["vecfleettest"],
                ...
                "spec": {
                    ...
                    "template": {
                        ...
                        "spec": {
                            ...
                            "containers": [{
                                "name": "vecfleet",
                                "image": "maddsystems/vecfleet:1.0.0",
    ...
    def _customize_manifests(self, templates: Dict[str, Any], client_name: str, params: Dict[str, Any]) -> None:
        ...
        for env in container["env"]:
            if env["name"] == "VECFLEET_URL":
                env["value"] = params["vecfleet_url"]
            if env["name"] == "VECFLEET_POST_URL":
                env["value"] = params["vecfleet_post_url"]
            if env["name"] == "VECFLEET_EMAIL":
                env["value"] = params["vecfleet_email"]
            if env["name"] == "VECFLEET_NAME":
                env["value"] = params["vecfleet_name"]
            if env["name"] == "VECFLEET_PASSWORD":
                env["value"] = params["vecfleet_password"]
            if env["name"] == "SERVICE":
                env["value"] = params["service"]
