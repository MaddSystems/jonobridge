...
class UnigisService(BaseService):
    @property
    def service_name(self) -> str:
        """Return the name of the service, which matches the database table name."""
        return "unigis"  # This must match the database table name
    ...
    def parameters_helpers(self) -> Dict[str, str]:
        ...
        return {
            "unigis_user": ["GPSCONTROLMX"],
            "unigis_key": ["MKF846tgf"],
            "unigis_url": ["http://hub.unisolutions.com.ar/hub/unigis/MAPI/SOAP/gps/service.asmx"],
            "plates_url": ["https://pluto.dudewhereismy.com.mx/imei/search?appId=2900"],
            "elastic_doc_name": ["unigis"],
                ...
                "spec": {
                    ...
                    "template": {
                        ...
                        "spec": {
                            ...
                            "containers": [{
                                "name": "unigis",
                                "image": "maddsystems/unigis:1.0.0",
    ...
    def _customize_manifests(self, templates: Dict[str, Any], client_name: str, params: Dict[str, Any]) -> None:
        ...
        for env in container["env"]:
            if env["name"] == "UNIGIS_USER":
                env["value"] = params["unigis_user"]
            if env["name"] == "UNIGIS_KEY":
                env["value"] = params["unigis_key"]
            if env["name"] == "UNIGIS_URL":
                env["value"] = params["unigis_url"]
