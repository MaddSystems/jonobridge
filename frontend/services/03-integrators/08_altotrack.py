...
class AltotrackService(BaseService):
    @property
    def service_name(self) -> str:
        """Return the name of the service, which matches the database table name."""
        return "altotrack"  # This must match the database table name
    ...
    def parameters_helpers(self) -> Dict[str, str]:
        ...
        return {
            "altotrack_proveedor": ["GPScontrol"],
            "altotrack_url": ["http://ws4.altotrack.com/WSPosiciones_Chep/WSPosiciones_Chep.svc?wsdl/IServicePositions"],
            "plates_url": ["https://pluto.dudewhereismy.com.mx/imei/search?appId=97"],
            "elastic_doc_name": ["Almanza_Chep_AltroTrack"],
                ...
                "spec": {
                    ...
                    "template": {
                        ...
                        "spec": {
                            ...
                            "containers": [{
                                "name": "altotrack",
                                "image": "maddsystems/altotrack:1.0.0",
    ...
    def _customize_manifests(self, templates: Dict[str, Any], client_name: str, params: Dict[str, Any]) -> None:
        ...
        for env in container["env"]:
            if env["name"] == "ALTOTRACK_PROVEEDOR":
                env["value"] = params["altotrack_proveedor"]
            if env["name"] == "ALTOTRACK_URL":
                env["value"] = params["altotrack_url"]
