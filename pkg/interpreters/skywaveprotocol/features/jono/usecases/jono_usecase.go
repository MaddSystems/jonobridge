...
func GetDataJono(data string) (string, error) {
	...
	// Check if this is ORBCOMM data (has Messages field)
	if _, exists := rawData["Messages"]; exists {
		return processORBCOMMData(rawData)
	}
...
// processORBCOMMData handles ORBCOMM satellite data structure
func processORBCOMMData(rawData map[string]interface{}) (string, error) {
	fmt.Printf("DEBUG: Processing ORBCOMM data: %+v\n", rawData)

	parsedModel := models.ParsedModel{
		ListPackets: make(map[string]models.Packet),
	}

	// Extract data from ORBCOMM structure
	if messages, ok := rawData["Messages"].(map[string]interface{}); ok {
		fmt.Printf("DEBUG: Found Messages: %+v\n", messages)
		if returnedMessages, ok := messages["ReturnedMessages"].([]interface{}); ok && len(returnedMessages) > 0 {
			fmt.Printf("DEBUG: Found ReturnedMessages: %+v\n", returnedMessages)
			// Process first message
			if firstMessage, ok := returnedMessages[0].(map[string]interface{}); ok {
				fmt.Printf("DEBUG: Processing first message: %+v\n", firstMessage)
				// Extract IMEI from MobileID
				if mobileID, ok := firstMessage["MobileID"].(string); ok {
					parsedModel.IMEI = &mobileID
					fmt.Printf("DEBUG: Extracted IMEI: %s\n", mobileID)
				}

				// Create packet from ORBCOMM data
				packet := models.Packet{
					EventCode: models.EventCode{
						Code: 31, // Heartbeat event for ORBCOMM
						Name: "ORBCOMM Heartbeat",
					},
					IoPortStatus: &models.IoPortsStatus{
						Port1: 0, Port2: 0, Port3: 0, Port4: 0,
						Port5: 0, Port6: 0, Port7: 0, Port8: 0,
					},
					NumberOfSatellites: getIntValue(firstMessage, "NumberOfSatellites"),
				}

				// Extract GPS data from Payload.Fields
				if payload, ok := firstMessage["Payload"].(map[string]interface{}); ok {
					fmt.Printf("DEBUG: Found Payload: %+v\n", payload)
					if fields, ok := payload["Fields"].(map[string]interface{}); ok {
						fmt.Printf("DEBUG: Found Fields: %+v\n", fields)
						if fieldList, ok := fields["Field"].([]interface{}); ok {
							fmt.Printf("DEBUG: Found Fields array: %+v\n", fieldList)
							for i, field := range fieldList {
								fmt.Printf("DEBUG: Processing field %d: %+v\n", i, field)
								if fieldMap, ok := field.(map[string]interface{}); ok {
									...
									case "Latitude":
										if lat, err := strconv.ParseFloat(value, 64); err == nil {
											packet.Latitude = &lat
											fmt.Printf("DEBUG: Set Latitude: %f\n", lat)
										}
									case "Longitude":
										if lon, err := strconv.ParseFloat(value, 64); err == nil {
											packet.Longitude = &lon
											fmt.Printf("DEBUG: Set Longitude: %f\n", lon)
										}
									case "Speed":
										if speed, err := strconv.ParseFloat(value, 64); err == nil {
											speedInt := int(speed)
											packet.Speed = &speedInt
											fmt.Printf("DEBUG: Set Speed: %d\n", speedInt)
										}
									case "Heading":
										if heading, err := strconv.ParseFloat(value, 64); err == nil {
											headingInt := int(heading)
											packet.Direction = &headingInt
											fmt.Printf("DEBUG: Set Direction: %d\n", headingInt)
										}
									case "EventTime":
										packet.Datetime = &value
										fmt.Printf("DEBUG: Set Datetime: %s\n", value)
									case "GPSFixAge":
										if fixAge, err := strconv.Atoi(value); err == nil {
											packet.NumberOfSatellites = fixAge
											fmt.Printf("DEBUG: Set NumberOfSatellites: %d\n", fixAge)
										}
									}
								}
							}
					...
					// Extract SIN from Payload
					if sin, ok := payload["SIN"].(float64); ok {
						// Use SIN as event code
						packet.EventCode = models.EventCode{
							Code: int(sin),
							Name: fmt.Sprintf("ORBCOMM Event %d", int(sin)),
						}
						fmt.Printf("DEBUG: Set EventCode from SIN: %f\n", sin)
					}
				}
	...
	jsonString, err := parsedModel.ToPrettyJSON()
	if err != nil {
		return "", fmt.Errorf("failed to convert ORBCOMM data to JSON: %w", err)
	}

	fmt.Printf("DEBUG: Final parsed model: %+v\n", parsedModel)
	return jsonString, nil
}
...
// 📌 Función auxiliar para obtener un int desde un map[string]interface{}
