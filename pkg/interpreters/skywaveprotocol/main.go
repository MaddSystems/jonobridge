...
// Map Skywave event names to MVT366 event codes
func mapSkywaveEventToMVT366(skywaveEvent string) (int, string) {
	eventMap := map[string]struct {
		...
		"DigInp2Hi":              {9, "SOS Inactive"}, // Input 1 Inactive = SOS Button Released
		"DigInp2Lo":              {1, "SOS Active"},   // Input 1 Active = SOS Button Pressed
		"HarshTurn":              {90, "Sharp Turn Left"},
		"IdlingStart":            {133, "Idle Overtime"},
		"IdlingEnd":              {134, "Idle Recovery"},
		"IgnitionOn":             {3, "Ignition on"},
		"IgnitionOff":            {11, "Ignition off"},
		"MovingStart":            {42, "Start Moving"},
		"MovingEnd":              {41, "Stop Moving"},
		"PowerBackup":            {22, "External Battery On"},
		"PowerMain":              {23, "External Battery Cut"},
		"Reset":                  {29, "Device Reboot"},
		"StationaryIntervalCell": {35, "Track By Time Interval"},
		"StationaryIntervalSat":  {35, "Track By Time Interval"},
		"MovingIntervalSat":      {35, "Track By Time Interval"},
		"DistanceCell":           {35, "Track By Time Interval"},
		"sleepSchedule":          {26, "Enter Sleep"},
		"terminalRegistration":   {31, "Heartbeat"},
		"modemRegistration":      {31, "Heartbeat"},
	}

	if event, ok := eventMap[skywaveEvent]; ok {
		return event.code, event.desc
	}

	// Default fallback
	return 35, "Track By Time Interval"
}

// Extract raw ReturnMessage XML for a specific message ID
func extractRawReturnMessage(xmlData []byte, messageID int64) string {
	xmlStr := string(xmlData)

	// Find the ReturnMessage containing this ID
	idStr := fmt.Sprintf("<ID>%d</ID>", messageID)
	idPos := strings.Index(xmlStr, idStr)
	if idPos == -1 {
		return ""
	}

	// Find the opening <ReturnMessage> tag before this ID
	searchStart := idPos
	for searchStart > 0 && !strings.HasPrefix(xmlStr[searchStart:], "<ReturnMessage>") {
		searchStart--
	}

	// Find the closing </ReturnMessage> tag after this ID
	searchEnd := strings.Index(xmlStr[idPos:], "</ReturnMessage>")
	if searchEnd == -1 {
		return ""
	}
	searchEnd += idPos + len("</ReturnMessage>")

	// Extract and return the complete ReturnMessage block
	return xmlStr[searchStart:searchEnd]
}
...
func parseSkyWaveXML(xmlData []byte) ([]PayloadBridge, error) {
	...
	for _, mess := range result.Messages.ReturnedMessages {
		// Skip messages without a payload name (empty payload)
		...
		// Only process messages that have GPS coordinates
...
func convertToMVT366(payload PayloadBridge) (string, error) {
	...
	// Map Skywave event to MVT366 event code
	eventCode, _ := mapSkywaveEventToMVT366(payload.Type)

	// Build MVT366 message
	mvt366 := fmt.Sprintf("%s,AAA,%d,%.6f,%.6f,%s,A,0,0,%.6f,%d,0.000000,21.232345,0.000000,0,0030|0030|0030|0030|0030,,,3,,,0,0",
		payload.MobileID,
		eventCode,
		lat,
		lon,
...
func convertSkyWaveToJonoProtocol(payload PayloadBridge) (string, error) {
	...
	// Map Skywave event to MVT366 event code
	eventCode, eventDesc := mapSkywaveEventToMVT366(payload.Type)

	// Set I/O Port Status based on event type
	// Following Python meitrack_sos_emulator.py logic:
	// - Normal: io_status="0200" -> Port1=0 (Input 1 OFF), Port2=1 (Input 2 ON)
	// - SOS Active (DigInp2Lo): io_status="0300" -> Port1=1 (Input 1 ON/SOS), Port2=1 (Input 2 ON)
	// - SOS Released (DigInp2Hi): io_status="0200" -> Port1=0 (Input 1 OFF), Port2=1 (Input 2 ON)
	// - IgnitionOn: Port3=1 (Input 3 ON/ignition active)
	// - IgnitionOff: Port3=0 (Input 3 OFF/ignition inactive)
	var ioPortStatus *models.IoPortsStatus
	switch payload.Type {
	case "DigInp2Lo": // SOS Button Pressed (Input 1 Active)
		ioPortStatus = &models.IoPortsStatus{
			Port1: 1, Port2: 1, Port3: 0, Port4: 0, Port5: 0, Port6: 0, Port7: 0, Port8: 0,
		}
	case "DigInp2Hi": // SOS Button Released (Input 1 Inactive)
		ioPortStatus = &models.IoPortsStatus{
			Port1: 0, Port2: 1, Port3: 0, Port4: 0, Port5: 0, Port6: 0, Port7: 0, Port8: 0,
		}
	case "IgnitionOn": // Ignition turned ON (Input 3 Active)
		ioPortStatus = &models.IoPortsStatus{
			Port1: 0, Port2: 1, Port3: 1, Port4: 0, Port5: 0, Port6: 0, Port7: 0, Port8: 0,
		}
	case "IgnitionOff": // Ignition turned OFF (Input 3 Inactive)
		ioPortStatus = &models.IoPortsStatus{
			Port1: 0, Port2: 1, Port3: 0, Port4: 0, Port5: 0, Port6: 0, Port7: 0, Port8: 0,
		}
	...
	packet := models.Packet{
		...
		EventCode:          models.EventCode{Code: eventCode, Name: eventDesc},
		Latitude:           &lat,
		Longitude:          &lon,
		Speed:              &speedInt,
...
func (m *MQTTClient) messageHandler(client mqtt.Client, msg mqtt.Message) {
	...
	if msg.Topic() == "skywave/xml" {
		...
		if m.verbose {
			vPrint("Found %d messages with GPS data in SkyWave XML", len(payloadBridges))
		}
		...
		for _, payload := range payloadBridges {
			if m.verbose {
				...
				// Find the corresponding ReturnMessage in the original XML
				rawMsg := extractRawReturnMessage(xmlBytes, payload.ID)
				if rawMsg != "" {
					// Print as hex string for easy copy-paste
					hexMsg := hex.EncodeToString([]byte(rawMsg))
					vPrint("Raw XML Message (hex): %s", hexMsg)
					//vPrint("Raw XML Message (decoded):\n%s", rawMsg)
				}
			}
