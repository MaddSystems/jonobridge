...
class ORBCOMMClient:
    ...
    def get_messages(self, mobile_id: str = None, hours_back: int = 24, minutes_back: int = None) -> Optional[List[Dict]]:
        """
        Retrieve messages from ORBCOMM for a specific device
        
        Args:
            mobile_id: Device ID (SAT ID) - optional, can filter by from_id
            hours_back: How many hours back to retrieve messages (deprecated, use minutes_back)
            minutes_back: How many minutes back to retrieve messages (takes precedence)
            
        Returns:
            List of parsed message dictionaries or None if failed
        """
        try:
            # Use minutes_back if provided, otherwise convert hours_back to minutes
            if minutes_back is not None:
                total_minutes_back = minutes_back
            else:
                total_minutes_back = hours_back * 60
            
            # Calculate time range for API request
            end_time = datetime.utcnow()
            start_time = end_time - timedelta(minutes=total_minutes_back)
            ...
            logger.info(f"Time range: {start_time.strftime('%Y-%m-%d %H:%M:%S')} to {end_time.strftime('%Y-%m-%d %H:%M:%S')} UTC")
            if mobile_id:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    logger.info(f"Parsed {len(messages)} messages from API time range")
                    return messages
...
def main():
    ...
    parser.add_argument('--hours-back', type=int, default=24, help='Hours of historical data to retrieve (deprecated)')
    parser.add_argument('--minutes-back', type=int, default=10, help='Minutes of historical data to retrieve')
    ...
    access_id = "70001184"
    password = "JEUTPKKH"
    from_id = "3898451771"  # Updated from_id from the device label 
    ...
    logger.info(f"Retrieving data from last {args.minutes_back} minutes")
    ...
    try:
        while True:
            ...
            # Get messages from ORBCOMM (use minutes_back for filtering)
            messages = orbcomm.get_messages(device_id, args.hours_back, args.minutes_back)
