...
func SendToElastic(logData ElasticLogData) (err error) {
	defer func() {
		if r := recover(); r != nil {
			log.Printf("PANIC RECOVERED in SendToElastic: %v", r)
			err = fmt.Errorf("panic in SendToElastic: %v", r)
		}
	}()
	...
	// Use environment variable for index name, fallback to "xpot"
	indexName := os.Getenv("ELASTIC_DOC_NAME")
	if indexName == "" {
		indexName = "xpot"
		log.Printf("⚠️  ELASTIC_DOC_NAME not set, using default index: '%s'", indexName)
	} else {
		indexName = ToSnakeCase(indexName)
		log.Printf("📋 Using Elasticsearch index from ELASTIC_DOC_NAME: '%s'", indexName)
	}
	elasticURL := fmt.Sprintf("%s/%s/_doc", elasticBaseURL, indexName)

	// Convert struct to map for dynamic sending
	data := map[string]interface{}{
		"client":       logData.Client,
		"messenger_id": logData.MessengerId,
		"date_time":    logData.DateTime,
		"type":         logData.Type,
		"lat":          logData.Lat,
		"lon":          logData.Lon,
		"alt":          logData.Alt,
	...
	log.Printf("Successfully sent data to Elasticsearch with status code %d", resp.StatusCode)
	return nil
}
