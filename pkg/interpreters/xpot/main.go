...
func processMessage(db *sql.DB, message Message) error {
	...
	// Calculate IMEI (same logic as used for Server1 transmission)
	...
	log.Printf("Attempting to send data to Elasticsearch for MessengerID: %s, IMEI: %s", message.MessengerID, imei)
	if err := utils.SendToElastic(logData); err != nil {
		log.Printf("❌ FAILED to send to Elasticsearch: %v", err)
		log.Printf("   Data that failed: MessengerID=%s, IMEI=%s, DateTime=%s, Type=%s, Lat=%f, Lon=%f, Alt=%d",
			message.MessengerID, imei, message.DateTime, message.MessageType,
			message.Latitude, message.Longitude, message.Altitude)
		// Don't return the error as we don't want to fail the main operation
	} else {
		log.Printf("✅ Successfully sent data to Elasticsearch for MessengerID: %s", message.MessengerID)
	}
