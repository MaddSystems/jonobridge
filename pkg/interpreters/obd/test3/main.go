...
func decodeTrackerResponse(data []byte) {
	...
	if messageID == 0x0200 {
		...
		if bodyStartIndex+26 <= bodyEndIndex {
			...
			positioningStatus := (status & 0x00000002) != 0 // Bit 1: 0=Not positioned, 1=Positioned
			accStatus := (status & 0x00000001) != 0         // Bit 0: 0=ACC off, 1=ACC on
			latDirection := (status & 0x00000004) != 0      // Bit 2: 0=North, 1=South
			lonDirection := (status & 0x00000008) != 0      // Bit 3: 0=East, 1=West
			oilWayStatus := (status & 0x00000400) != 0      // Bit 10: 0=Normal, 1=Cut
			circuitStatus := (status & 0x00000800) != 0     // Bit 11: 0=Normal, 1=Disconnected
			doorLockStatus := (status & 0x00001000) != 0    // Bit 12: 0=Unlocked, 1=Locked
			maintenanceMode := (status & 0x00002000) != 0   // Bit 13: 0=Normal, 1=Maintenance
			wifiStatus := (status & 0x00004000) != 0        // Bit 14: 0=Off, 1=On

			// Extract alarm status information
			emergencyAlarm := (alarmFlag & 0x00000001) != 0        // Bit 0: Emergency warning
			overSpeedAlarm := (alarmFlag & 0x00000002) != 0        // Bit 1: Over-speed alarm
			fatigueDrivingAlarm := (alarmFlag & 0x00000004) != 0   // Bit 2: Fatigue driving
			dangerWarning := (alarmFlag & 0x00000008) != 0         // Bit 3: Danger warning
			gnssModuleFailure := (alarmFlag & 0x00000010) != 0     // Bit 4: GNSS module failure
			gnssAntennaDisconnect := (alarmFlag & 0x00000020) != 0 // Bit 5: GNSS antenna disconnected
			gnssAntennaShort := (alarmFlag & 0x00000040) != 0      // Bit 6: GNSS antenna short circuit
			terminalUnderVoltage := (alarmFlag & 0x00000080) != 0  // Bit 7: Terminal under voltage
			terminalPowerDown := (alarmFlag & 0x00000100) != 0     // Bit 8: Terminal power down
			terminalLcdFailure := (alarmFlag & 0x00000200) != 0    // Bit 9: LCD failure
			ttsModuleFailure := (alarmFlag & 0x00000400) != 0      // Bit 10: TTS module failure
			cameraFailure := (alarmFlag & 0x00000800) != 0         // Bit 11: Camera failure
			overSpeedWarning := (alarmFlag & 0x00002000) != 0      // Bit 13: Overspeed warning
			...
			latitudeDegrees := float64(latitude) / 1000000
			longitudeDegrees := float64(longitude) / 1000000

			if latDirection {
				latitudeDegrees = -latitudeDegrees // South is negative
			}

			if lonDirection {
				longitudeDegrees = -longitudeDegrees // West is negative
			}

			log.Printf("Latitude: %.6f° (%s)", math.Abs(latitudeDegrees),
				map[bool]string{false: "N", true: "S"}[latDirection])
			log.Printf("Longitude: %.6f° (%s)", math.Abs(longitudeDegrees),
				map[bool]string{false: "E", true: "W"}[lonDirection])

			// Elevation will be available if there's additional info with ID=0x01
			// Will handle this in the additional info section

			log.Printf("Speed: %.1f km/h", float64(speed)/10)
			log.Printf("Direction: %d°", direction)
			log.Printf("Datetime: %s", timestamp)

			// Vehicle Status
			log.Println("------- VEHICLE STATUS -------")
			log.Printf("Positioning: %s", map[bool]string{false: "No GPS Fix", true: "GPS Fixed"}[positioningStatus])
			log.Printf("ACC: %s", map[bool]string{false: "OFF", true: "ON"}[accStatus])
			log.Printf("Oil-way: %s", map[bool]string{false: "Connected", true: "Cut"}[oilWayStatus])
			log.Printf("Circuit: %s", map[bool]string{false: "Connected", true: "Cut"}[circuitStatus])
			log.Printf("Door Lock: %s", map[bool]string{false: "Unlocked", true: "Locked"}[doorLockStatus])
			log.Printf("Mode: %s", map[bool]string{false: "Normal", true: "Maintenance"}[maintenanceMode])
			log.Printf("WiFi: %s", map[bool]string{false: "OFF", true: "ON"}[wifiStatus])

			// Alarm Status
			if alarmFlag > 0 {
				log.Println("------- ALARM STATUS -------")
				if emergencyAlarm {
					log.Println("- Emergency alarm triggered")
				}
				if overSpeedAlarm {
					log.Println("- Over-speed alarm")
				}
				if fatigueDrivingAlarm {
					log.Println("- Fatigue driving alarm")
				}
				if dangerWarning {
					log.Println("- Danger warning")
				}
				if gnssModuleFailure {
					log.Println("- GNSS module failure")
				}
				if gnssAntennaDisconnect {
					log.Println("- GNSS antenna disconnected")
				}
				if gnssAntennaShort {
					log.Println("- GNSS antenna short circuit")
				}
				if terminalUnderVoltage {
					log.Println("- Terminal under voltage")
				}
				if terminalPowerDown {
					log.Println("- Terminal power down")
				}
				if terminalLcdFailure {
					log.Println("- Terminal LCD failure")
				}
				if ttsModuleFailure {
					log.Println("- TTS module failure")
				}
				if cameraFailure {
					log.Println("- Camera failure")
				}
				if overSpeedWarning {
					log.Println("- Over-speed warning")
				}
			}
		} else {
		...
		if bodyStartIndex+26 < bodyEndIndex {
			...
			log.Println("------- ADDITIONAL INFO -------")

			var mileage float64
			var fuelPercentage float64
			var gsmlSignalStrength int
			...
			for len(additionalInfo) > 2 {
				...
				// Process specific additional info IDs
				switch infoID {
				case 0x01: // Mileage, 4 bytes
					if infoLength == 4 {
						mileageValue := binary.BigEndian.Uint32(infoData)
						mileage = float64(mileageValue) / 10 // in km
						log.Printf("Mileage: %.1f km", mileage)
					}

				case 0x02: // Fuel percentage, 2 bytes
					if infoLength == 2 {
						fuelValue := binary.BigEndian.Uint16(infoData)
						fuelPercentage = float64(fuelValue) / 10 // in percentage
						log.Printf("Fuel: %.1f%%", fuelPercentage)
					}

				case 0x03: // Altitude, 2 bytes
					if infoLength == 2 {
						elevationValue = int(binary.BigEndian.Uint16(infoData))
						log.Printf("Altitude: %d meters", elevationValue)
					}

				case 0x14: // GSM signal strength, 1 byte
					if infoLength == 1 {
						gsmlSignalStrength = int(infoData[0])
						log.Printf("GSM Signal: %d dBm", gsmlSignalStrength)
					}

				case 0xEA: // Basic data flow with sub-IDs
					if len(infoData) >= 3 {
						...
						if len(infoData) >= 3+int(subLength) {
							...
							switch subID {
							case 0x0014: // CSQ value (network signal strength)
								if subLength == 1 {
									csqValue := subData[0]
									log.Printf("CSQ Value: %d", csqValue)
								}

							case 0x0018: // Number of satellites
								if subLength == 1 {
									numSatellites = int(subData[0])
									log.Printf("Number of Satellites: %d", numSatellites)
								}

							default:
								log.Printf("Basic data - SubID: 0x%04X, Length: %d, Data: %X",
				...
				case 0xEB: // Car extended data flow
					log.Printf("Car extended data - Length: %d, Data: %X", infoLength, infoData)
					// Further processing based on sub-IDs if needed

				case 0xEC: // Truck extended data flow
					log.Printf("Truck extended data - Length: %d, Data: %X", infoLength, infoData)
					// Further processing based on sub-IDs if needed

				case 0xED: // New energy vehicle data
					log.Printf("New energy vehicle data - Length: %d, Data: %X", infoLength, infoData)
					// Further processing based on sub-IDs if needed

				case 0xEE: // Extended peripheral data
					log.Printf("Extended peripheral data - Length: %d, Data: %X", infoLength, infoData)
					// Further processing based on sub-IDs if needed

				case 0xFA: // Alarm status and descriptions
					log.Printf("Alarm status data - Length: %d, Data: %X", infoLength, infoData)
					// Further processing based on sub-IDs if needed

				case 0xFB: // Base station data flow
					log.Printf("Base station data - Length: %d, Data: %X", infoLength, infoData)
					// Further processing based on sub-IDs if needed

				case 0xFC: // WiFi data flow
					log.Printf("WiFi data - Length: %d, Data: %X", infoLength, infoData)
					// Further processing based on sub-IDs if needed

				default:
					log.Printf("Additional Info ID: 0x%02X, Length: %d, Data: %X",
