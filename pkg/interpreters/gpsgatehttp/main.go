...
func sendTelegramNotification(data map[string]interface{}) {
			...
			if err != nil {
				...
				if resp.StatusCode == http.StatusOK {
					...
					if err := json.NewDecoder(resp.Body).Decode(&egoResp); err != nil {
						...
						// Extract "Nombre de Aplicación server 1"
						if appNameEGO, exists := egoResp["Nombre de Aplicación server 1"]; exists {
	...
	// Try fields in priority order: USER_NAME → ASSIGNED_VEHICLE_DESC → ASSIGNED_VEHICLE_NAME → DRIVER_DESCRIPTION
...
func main() {
	...
	// Start periodic logging to track if app is still running
	go func() {
		...
		for {
			...
			case <-ticker.C:
				...
				currentCount := messageCounter
				processingMutex.RUnlock()
				log.Printf("App is still running, messageCounter: %d", currentCount)
			}
		}
	}()
	...
	log.Printf("About to start processGPSgateData (main loop)")
	err := processGPSgateData()
	if err != nil {
		log.Printf("ERROR: processGPSgateData returned: %v", err)
		log.Printf("EXITING with error code 2 due to fatal error in processGPSgateData")
		os.Exit(2)
	}
	log.Printf("processGPSgateData exited normally (should not happen in normal operation)")
}
