...
func ParseAAAFields(aaaFields *models.AAAModel) (string, error) {
	...
	if len(parts) >= 18 {
		...
		aaaFields.Latitude, _ = strconv.ParseFloat(parts[1], 64)
		aaaFields.Longitude, _ = strconv.ParseFloat(parts[2], 64)
		...
		aaaFields.HDOP, _ = strconv.ParseFloat(parts[9], 64) // Changed from Hdop to HDOP
		aaaFields.Altitude, _ = strconv.ParseFloat(parts[10], 64)
