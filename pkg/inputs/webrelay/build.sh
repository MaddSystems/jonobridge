...
cd "${REPO_ROOT}/pkg/inputs/webrelay"
go build

# Clean up any existing Docker images with the webrelaytrack name
echo "Removing old Docker images..."
docker images --filter=reference="*webrelay*" --format "{{.ID}}" | xargs -r docker rmi -f
...
docker build -t webrelay -f ./pkg/inputs/webrelay/Dockerfile .

# Tag and push the image
echo "Tagging Docker image..."
docker tag webrelay maddsystems/webrelay:1.0.0
echo "Pushing Docker image..."
docker push maddsystems/webrelay:1.0.0
