...
func processHttpData() error {
	urlsEnv := getRequiredEnv("HTTP_URL")

	// Split URLs by comma and trim whitespace
	urls := strings.Split(urlsEnv, ",")

	utils.VPrint("Found %d URL(s) to process\n", len(urls))
	...
	for i, url := range urls {
		...
		utils.VPrint("[%d/%d] Fetching data from HTTP GET: %s\n", i+1, len(urls), url)

		resp, err := http.Get(url)
		if err != nil {
			lastError = fmt.Errorf("error making GET request to %s: %v", url, err)
			log.Printf("Error fetching URL [%d/%d]: %v", i+1, len(urls), lastError)
			continue
		}
		...
		if err != nil {
			lastError = fmt.Errorf("error reading response body from %s: %v", url, err)
			log.Printf("Error reading response [%d/%d]: %v", i+1, len(urls), lastError)
			continue
		}
		...
		utils.VPrint("[%d/%d] Data fetched from HTTP:\n%s\n", i+1, len(urls), escapedDump)

		// Publish to MQTT
		if token := mqttClient.Publish("http/get", 0, false, hexString); token.Wait() && token.Error() != nil {
			lastError = fmt.Errorf("error publishing to MQTT for %s: %v", url, token.Error())
			log.Printf("Error publishing to MQTT [%d/%d]: %v", i+1, len(urls), lastError)
			continue
		}

		utils.VPrint("[%d/%d] Successfully published to MQTT\n", i+1, len(urls))
		successCount++
	}

	utils.VPrint("Completed processing %d/%d URLs successfully\n", successCount, len(urls))

	// Return error only if all URLs failed
...
func processSkyWaveData(config *SkyWaveConfig, pollInterval time.Duration) error {
	...
	for {
		...
		// Print the full XML response in verbose mode
		utils.VPrint("=== SkyWave API Response (Page %d) ===", pageNum)
		utils.VPrint("%s", string(body))
		utils.VPrint("=== End of Response (Page %d) ===\n", pageNum)
		...
		utils.VPrint("=== Hex String (Page %d) ===", pageNum)
		utils.VPrint("%s", hexString)
		utils.VPrint("=== End of Hex String (Page %d) ===\n", pageNum)
