...
import (
	"encoding/json"
	"fmt"
	"proxy/features/meitrack_protocol/config"
...
func ParseAAAFields(aaaFields *models.AAAModel) (string, error) {
	...
	if len(parts) >= 18 {
		...
		aaaFields.Hdop, _ = strconv.ParseFloat(parts[9], 32)
		aaaFields.Altitude, _ = strconv.ParseFloat(parts[10], 32)
		aaaFields.Mileage, _ = strconv.Atoi(parts[11])
