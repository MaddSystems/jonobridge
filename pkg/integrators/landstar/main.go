...
import (
	landstarintegrator "landstar/features/landstar_integrator"
	"flag"
	"fmt"
	"log"
	"os"
...
func main() {
	...
	// Configure and initialize landstar before anything else
	landstarToken := os.Getenv("LANDSTAR_TOKEN")
	landstarURL := os.Getenv("LANDSTAR_URL")
	fmt.Println("token length:", len(landstarToken))
	fmt.Println("url:", landstarURL)

	// Initialize the integrator explicitly
	landstarintegrator.Initialize()
	...
	clientID := fmt.Sprintf("LANDSTAR_%s_%s_%d",
		"LANDSTAR",
		os.Getenv("HOSTNAME"),
	...
	opts.SetDefaultPublishHandler(func(client mqtt.Client, msg mqtt.Message) {
		...
		landstarintegrator.Init(string(msg.Payload()))
