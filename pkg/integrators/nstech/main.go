...
import (
	...
	"fmt"
	"log"
	nstechintegrator "nstech/features/nstech_integrator"
	"os"
	"time"
...
func main() {
	...
	clientID := fmt.Sprintf("NSTECH_%s_%s_%d",
		"NSTECH",
		os.Getenv("HOSTNAME"),
	...
	opts.SetDefaultPublishHandler(func(client mqtt.Client, msg mqtt.Message) {
		...
		// Main routine to process the incoming data with error handling
		...
		if err := nstechintegrator.Init(string(msg.Payload())); err != nil {
			utils.VPrint("Error processing MQTT message (non-fatal): %v", err)
