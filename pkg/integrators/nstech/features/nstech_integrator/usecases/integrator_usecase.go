...
func RunIntegrator(imei, dataStr string) error {
	...
	if err := data.ProcessAndSendNstech(platesStr, ecoStr, vinStr, dataStr); err != nil {
		utils.VPrint("Error processing and sending data (non-fatal): %v", err)
		// Don't return error to avoid crashing the container
	}
