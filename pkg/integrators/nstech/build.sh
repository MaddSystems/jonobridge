...
cd "${REPO_ROOT}/pkg/integrators/nstech"
go build

# Clean up any existing Docker images with the nstechtrack name
echo "Removing old Docker images..."
docker images --filter=reference="*nstech*" --format "{{.ID}}" | xargs -r docker rmi -f
...
docker build -t nstech -f ./pkg/integrators/nstech/Dockerfile .

# Tag and push the image
echo "Tagging Docker image..."
docker tag nstech maddsystems/nstech:1.0.0
echo "Pushing Docker image..."
docker push maddsystems/nstech:1.0.0
