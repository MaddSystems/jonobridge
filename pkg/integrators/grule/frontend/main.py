            ...
            try:
                ...
                if os.path.exists(yaml_path):
                    try:
                        ...
                        print(f"⚠️ Error loading manifest {yaml_path}: {e}")
...
def new_rule():
    ...
    if request.method == 'POST':
        ...
        print(f"📋 [Frontend] Form fields received:")
        for key in request.form:
            value = request.form[key]
            print(f"   {key}: {len(value) if isinstance(value, str) else value} chars")
        
        audit_manifest = request.form.get('audit_manifest', '')
        print(f"📝 [Frontend] Creating rule. Manifest length: {len(audit_manifest)}")
        if audit_manifest:
            print(f"📝 [Frontend] Manifest preview: {audit_manifest[:100]}")
        else:
            print(f"⚠️  [Frontend] WARNING: audit_manifest is EMPTY!")
        ...
        print(f"📤 [Frontend] Sending to API: {API_BASE_URL}/api/rules")
        print(f"   Rule data keys: {rule_data.keys()}")
        print(f"   audit_manifest in payload: {len(rule_data.get('audit_manifest', ''))} chars")
        try:
...
def get_audit_details():
    """Proxy audit details endpoint to Go backend for alert details by IMEI."""
    try:
        imei = request.args.get('imei', '')
        limit = request.args.get('limit', '50')
        page = request.args.get('page', '1')
        ...
        response = requests.get(f"{API_BASE_URL}/api/audit/details", params=params)
        response.raise_for_status()
        return jsonify(response.json())
    except requests.exceptions.RequestException as e:
        return jsonify({"error": str(e)}), 500
