...
def print_test_prediction():
    ...
    print(f"\n{'='*80}")
    print("🔮 DEFCON TEST PREDICTION - THREADED VERSION")
    print(f"{'='*80}\n")
    ...
    for cfg in IMEI_CONFIG:
        ...
        print(f"   Start delay: {cfg['start_delay']}s")
        ...
        if gsm < 15:
            ...
            if inside_safe_zone:
                ...
                if will_be_offline:
                    ...
                    trigger_time_offset = cfg['start_delay'] + (WARMUP_PACKETS * 1) + (packets_needed_for_offline * PACKET_INTERVAL)
                    print(f"   ⏰ Expected DEFCON 4 trigger: ~{trigger_time_offset}s after test start\n")
    ...
    for imei in defcon4_expected:
        ...
        packets_needed_for_offline = int(5 * 60 / PACKET_INTERVAL)
        trigger_time = cfg['start_delay'] + (WARMUP_PACKETS * 1) + (packets_needed_for_offline * PACKET_INTERVAL)
        print(f"      ✅ {cfg['name']} - {imei} (~{trigger_time}s)")
    ...
    if len(defcon4_expected) >= 2:
        ...
        if len(defcon4_expected) == 2:
            cfg1 = next(c for c in IMEI_CONFIG if c['imei'] == defcon4_expected[0])
            cfg2 = next(c for c in IMEI_CONFIG if c['imei'] == defcon4_expected[1])
            time_diff = abs(cfg1['start_delay'] - cfg2['start_delay'])
            print(f"   ⏱️  Time difference between DEFCON 4 alerts: ~{time_diff} seconds")
    else:
...
def send_packet(imei, output, label):
    ...
    try:
        ...
        s.close()
        print(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] {label} - IMEI: {imei} - Sent OK")
    except Exception as e:
        print(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] {label} - IMEI: {imei} - ERROR: {e}")

def imei_worker(cfg, start_time):
    """Worker thread for each IMEI"""
    # Wait for start_delay
    delay_until = start_time + timedelta(seconds=cfg['start_delay'])
    wait = (delay_until - datetime.now()).total_seconds()
    if wait > 0:
        print(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] Thread {cfg['name']} waiting {wait:.1f}s before start...")
        time.sleep(wait)
    
    print(f"\n[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] 🚀 Starting {cfg['name']}")
    
    # PHASE 1: WARMUP (11 VALID PACKETS)
    print(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] --- {cfg['name']}: WARMUP Phase ---")
    for step in range(WARMUP_PACKETS):
        ...
        send_packet(cfg["imei"], out, f"{cfg['name']} WARMUP {step+1}/{WARMUP_PACKETS}")
        time.sleep(1)  # Fast warmup
    
    # PHASE 2: SIMULATION (24 INVALID PACKETS)
    print(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] --- {cfg['name']}: SIMULATION Phase ---")
    for step in range(INVALID_PACKETS):
        if step >= cfg["stop_at_step"]:
            print(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] {cfg['name']} - Stopping at step {step}")
            break
        ...
        out = payload(
            ...
            speed="100",  # High speed for DEFCON 2
            direction="0", accuracy="0", altitude="0", mileage="0", runtime="1348",
            mcc="334", mnc="020", lac="1234", cellId="5678", 
            portStatus="0000", AD1="0000", AD2="0000", AD3="0000", battery="100", AD5="0000", eventInfo="00000000"
        )
        send_packet(cfg["imei"], out, f"{cfg['name']} SIMUL {step+1}/{INVALID_PACKETS} (T:{cfg['target']})")
        time.sleep(PACKET_INTERVAL)
    
    print(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] ✅ {cfg['name']} - Thread completed")
...
def run_test():
    ...
    print(f"\n{'='*60}")
    print(f"🚀 STARTING THREADED MULTI-IMEI TEST AT {datetime.now()}")
    print(f"Targets: 2x Defcon 4 (15s apart), 1x Defcon 2, 1x Defcon 1")
    print(f"{'='*60}\n")
    
    start_time = datetime.now()
    threads = []
    
    # Create and start a thread for each IMEI
    for cfg in IMEI_CONFIG:
        thread = threading.Thread(target=imei_worker, args=(cfg, start_time), name=cfg['name'])
        thread.start()
        threads.append(thread)
        print(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] Created thread for {cfg['name']}")
    
    # Wait for all threads to complete
    print(f"\n[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] Waiting for all threads to complete...\n")
    for thread in threads:
        thread.join()
    
    print(f"\n{'='*60}")
    print(f"✅ ALL THREADS COMPLETED AT {datetime.now()}")
    print(f"{'='*60}\n")
