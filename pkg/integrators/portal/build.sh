...
cd "${REPO_ROOT}/pkg/integrators/portal"
go build

# Clean up any existing Docker images with the portaltrack name
echo "Removing old Docker images..."
docker images --filter=reference="*portal*" --format "{{.ID}}" | xargs -r docker rmi -f
...
docker build -t portal -f ./pkg/integrators/portal/Dockerfile .

# Tag and push the image
echo "Tagging Docker image..."
docker tag portal maddsystems/portal:1.0.0
echo "Pushing Docker image..."
docker push maddsystems/portal:1.0.0
