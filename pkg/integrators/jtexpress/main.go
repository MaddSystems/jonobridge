...
import (
	"flag"
	"fmt"
	jtexpressintegrator "jtexpress/features/jtexpress_integrator"
	"log"
	"os"
	"time"
...
func main() {
	...
	// Configure and initialize jtexpress before anything else
	...
	fmt.Println("JT Express URL:", jtexpressURL)
	fmt.Println("JT Express API Account:", jtexpressAPIAccount)
	fmt.Println("JT Express Private Key length:", len(jtexpressPrivateKey))

	// Initialize the integrator explicitly
	jtexpressintegrator.Initialize()
	...
	clientID := fmt.Sprintf("JTEXPRESS_%s_%s_%d",
		"JTEXPRESS",
		os.Getenv("HOSTNAME"),
	...
	opts.SetDefaultPublishHandler(func(client mqtt.Client, msg mqtt.Message) {
		...
		jtexpressintegrator.Init(string(msg.Payload()))
