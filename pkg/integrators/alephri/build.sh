...
cd "${REPO_ROOT}/pkg/integrators/alephri"
go build

# Clean up any existing Docker images with the alephritrack name
echo "Removing old Docker images..."
docker images --filter=reference="*alephri*" --format "{{.ID}}" | xargs -r docker rmi -f
...
docker build -t alephri -f ./pkg/integrators/alephri/Dockerfile .

# Tag and push the image
echo "Tagging Docker image..."
docker tag alephri maddsystems/alephri:1.0.0
echo "Pushing Docker image..."
docker push maddsystems/alephri:1.0.0
