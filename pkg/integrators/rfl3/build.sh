...
cd "${REPO_ROOT}/pkg/integrators/rfl3"
go build

# Clean up any existing Docker images with the rfl3track name
echo "Removing old Docker images..."
docker images --filter=reference="*rfl3*" --format "{{.ID}}" | xargs -r docker rmi -f
...
docker build -t rfl3 -f ./pkg/integrators/rfl3/Dockerfile .

# Tag and push the image
echo "Tagging Docker image..."
docker tag rfl3 maddsystems/rfl3:1.0.0
echo "Pushing Docker image..."
docker push maddsystems/rfl3:1.0.0
