...
func main() {
	...
	flag.Parse()
	// rc_user := os.Getenv("USER")
	// rc_password := os.Getenv("PASSWORD")
	// rc_url := os.Getenv("URL")
	// fmt.Println("user:", rc_user)
	// fmt.Println("password:", rc_password)
	// fmt.Println("URL:", rc_url)
	...
	clientID := fmt.Sprintf("VECFLEET_%s_%s_%d",
		"VECFLEET",
		os.Getenv("HOSTNAME"),
	...
	opts.SetDefaultPublishHandler(func(client mqtt.Client, msg mqtt.Message) {
		...
		err := vecfleetintegrator.Init(string(msg.Payload()))
		if err != nil {
			fmt.Println(err)
		}
