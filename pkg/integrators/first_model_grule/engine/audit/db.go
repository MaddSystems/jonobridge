...
func GetFrameTimeline(imei, ruleName string, limit int) ([]map[string]interface{}, error) {
	...
	for rows.Next() {
		...
		var ruleID int64
		var ruleName, stageReached, stopReason, geofenceEval string
		...
		// Construir el resultado, manejando la fecha nula
		...
		if executionTime.Valid {
			frame["execution_time"] = executionTime.Time
		} else {
			frame["execution_time"] = nil
		}
...
// GetFrameTimelinePaginated obtiene un timeline de frames con paginación, ordenamiento y búsqueda
func GetFrameTimelinePaginated(imei string, limit, offset int, sortBy, sortOrder, ruleName string) ([]map[string]interface{}, int, error) {
	...
	if sortOrder != "ASC" && sortOrder != "DESC" {
		sortOrder = "ASC" // Por defecto ascendente para timeline
	}

	// Construir cláusula WHERE
	var whereConditions []string
	var args []interface{}
	whereConditions = append(whereConditions, "imei = ?")
	...
	countQuery := fmt.Sprintf("SELECT COUNT(*) FROM rule_execution_state %s", whereClause)
	var total int
	err := db.QueryRow(countQuery, args...).Scan(&total)
	if err != nil {
		return nil, 0, fmt.Errorf("error counting paginated frame timeline: %v", err)
	}
	...
	rows, err := db.Query(query, paginatedArgs...)
	if err != nil {
		return nil, 0, fmt.Errorf("error querying paginated frame timeline: %v", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		var id, stepNumber, bufferSize int
		var ruleID int64
		var ruleNameStr, stageReached, stopReason, geofenceEval string
		var executionTime sql.NullTime // Usar sql.NullTime para manejar fechas NULL
		var metricsReady bool
		var contextSnapshotJSON, componentsExecutedJSON, componentDetailsJSON []byte

		err := rows.Scan(&id, &ruleID, &ruleNameStr, &componentsExecutedJSON, &componentDetailsJSON, &stepNumber, &stageReached, &stopReason,
			&bufferSize, &metricsReady, &geofenceEval, &contextSnapshotJSON, &executionTime)
		if err != nil {
			log.Printf("⚠️  Error scanning paginated frame: %v", err)
			continue
		}
		...
		if err := json.Unmarshal(contextSnapshotJSON, &snapshot); err != nil {
			log.Printf("⚠️  Error unmarshaling paginated snapshot: %v", err)
			snapshot = map[string]interface{}{"error": "invalid JSON"}
		}

		if err := json.Unmarshal(componentsExecutedJSON, &componentsExecuted); err != nil {
			log.Printf("⚠️  Error unmarshaling paginated components: %v", err)
			componentsExecuted = []string{}
		}

		if err := json.Unmarshal(componentDetailsJSON, &componentDetails); err != nil {
			log.Printf("⚠️  Error unmarshaling paginated component details: %v", err)
			componentDetails = map[string]interface{}{}
		}
...
// GetSnapshotByID obtiene el context_snapshot para un ID de frame específico
func GetSnapshotByID(id int) (map[string]interface{}, error) {
	...
	query := "SELECT context_snapshot FROM rule_execution_state WHERE id = ?"

	err := db.QueryRow(query, id).Scan(&contextSnapshotJSON)
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, fmt.Errorf("frame with ID %d not found", id)
		}
		return nil, fmt.Errorf("error querying snapshot by ID: %v", err)
	}

	var snapshot map[string]interface{}
	if err := json.Unmarshal(contextSnapshotJSON, &snapshot); err != nil {
		log.Printf("⚠️  Error unmarshaling snapshot for ID %d: %v", id, err)
		return nil, fmt.Errorf("error unmarshaling snapshot JSON: %v", err)
	}
