...
func (wp *WorkerPool) workerRoutine(imei string, ch chan *models.JonoModel) {
	gID := getGoroutineID()
	for jono := range ch {
		log.Printf("[GID:%s] 🔄 Worker procesando %d paquetes para IMEI: %s", gID, len(jono.ListPackets), imei)
...
// TODO TODO estado mutable (como Cond1Passed) debe vivir en Property con sincronización
...
// ProcessJonoMessage desempaqueta JSON y envía al worker correspondiente
...
func executeRulesForPacket(wrapper PacketWrapper, packetKey string, kbs []*ast.KnowledgeBase, gID string) {
	...
	for i, kb := range kbs {
		...
		if err != nil {
			...
			// También loggeamos desde Jono por compatibilidad (ahora usando property)
