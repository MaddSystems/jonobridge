...
// AddEntry - REPLICA EXACTA líneas 60-69 GPSgate
func (b *FixedCircularBuffer) AddEntry(entry BufferEntry) bool {
	...
	// SIEMPRE calcular isValid (réplica línea 82)
	entry.IsValid = (entry.PositioningStatus == "true" || entry.PositioningStatus == "A")

	// VALIDACIÓN DE SECUENCIA TEMPORAL:
	// Si el buffer tiene datos, verificar que la nueva trama sea POSTERIOR a la última almacenada.
	// Esto evita que tramas antiguas ("descarga de memoria") corrompan el buffer circular.
	if b.size > 0 {
		...
		if !entry.Datetime.After(lastEntryTime) {
			...
			// Retornamos el estado actual de "lleno" sin modificar nada
			return b.size >= 10
		}
	}

	if b.size < 10 {
		// CASO 1: Buffer no lleno - solo push (líneas 61-62 GPSgate)
		...
		// CASO 2: Buffer lleno - shift + push (líneas 63-65 GPSgate)
		for i := 0; i < 9; i++ {
...
// GetEntriesInTimeWindow90Min - Filtra buffer por ventana 90min (réplica forEach línea 88)
func (b *FixedCircularBuffer) GetEntriesInTimeWindow90Min() []BufferEntry {
	...
	// Réplica forEach de GPSgate (línea 88) - PASO 1: Filtrar por tiempo
	for i := 0; i < 10; i++ {
		entry := b.entries[i]
		diferencia := now.Sub(entry.Datetime).Minutes() // líneas 90-92

		if diferencia < 90 { // réplica línea 94: if(diferencia < maximumtime)
...
// CalculateAverageSpeed90Min - Calcula promedio del filtrado (réplica línea 96+105)
func (b *FixedCircularBuffer) CalculateAverageSpeed90Min() float64 {
	...
	for _, entry := range filtered {
		totalSpeed += float64(entry.Speed) // réplica: averageKm += data["km"]
	}

	return totalSpeed / float64(len(filtered)) // réplica línea 105
}

// CalculateAverageGSMLast5 - Calcula GSM promedio últimos 5 del filtrado (réplica líneas 97-98 + 106)
func (b *FixedCircularBuffer) CalculateAverageGSMLast5() float64 {
	...
	if len(filtered) < 5 {
		return 0 // Necesita al menos 5 entradas para promediar últimos 5
	}

	// PASO 2: Sumar SOLO los últimos 5 elementos y dividir entre 5
	totalGSM := 0.0
...
func InitializeFixedBuffers(retention time.Duration) {
	...
	log.Println("🧠 Memory Buffer inicializado: 10 posiciones fijas por IMEI")

	// CRÍTICO: Lanzar goroutine de limpieza para evitar memory leak en producción
	// Sin esto → proceso muere en 3-12 meses con millones de IMEIs
...
// startCleanupRoutine - CRÍTICO: Limpia buffers inactivos cada 24 horas
// Previene memory leak con millones de IMEIs históricos
func (bm *BufferManager) startCleanupRoutine() {
	ticker := time.NewTicker(24 * time.Hour) // Cleanup cada 24 horas
	defer ticker.Stop()

	log.Println("🧹 Cleanup routine iniciado: cada 24 horas")
...
func (bm *BufferManager) cleanupInactiveBuffers() {
	...
	for imei, buffer := range bm.buffers {
		// Si no ha sido usado por más tiempo que retention, eliminarlo
	...
	log.Printf("🧹 Cleanup completado: %d buffers eliminados, %d → %d activos",
		deletedCount, initialCount, finalCount)
}

// FUNCIONES PÚBLICAS para usar en persistent_state.go:
...
func AddToBuffer(imei string, speed int64, gsmSignal int64, datetime time.Time,
	...
	if GlobalBufferManager == nil {
		log.Printf("⚠️ BufferManager no inicializado")
		return false
	}
	...
	hasExactly10 := buffer.AddEntry(entry) // Réplica doit de GPSgate
	return hasExactly10
}
...
// GetBufferEntries retorna una copia del buffer circular para auditoría (thread-safe)
func GetBufferEntries(imei string) []BufferEntry {
	...
	// Copiar solo las entradas válidas (0 hasta buffer.size)
