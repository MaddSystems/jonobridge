...
def send_defcon_test():
    server = "jonobridge.madd.com.mx"
    port = 8056
    
    # Configuración del test
    imei = "864352045580768"
    packet_interval = 15 # Segundos entre paquetes (rápido para pruebas)
    
    # FASE 1: LLENAR BUFFER (10 paquetes VÁLIDOS)
    # Esto asegura que tengamos historia válida para cuando falle el GPS
    ...
    # Alinear al próximo segundo exacto para empezar limpio
    ...
    # FASE 2: PROVOCAR DEFCON 1 -> 2 (Paquetes INVÁLIDOS)
    # Al tener buffer lleno (10) y llegar una trama inválida (V),
    # debería pasar DEFCON 1 y entrar a DEFCON 2 (Check Offline)
    print("\n=== FASE 2: Provocando DEFCON 1 -> 2 (5 paquetes INVÁLIDOS) ===")
    num_invalid_packets = 5
    ...
    for i in range(num_invalid_packets):
        ...
        # Mantenemos lat/lon congelada o 0 (simulando pérdida)
        utc = datetime.utcnow().strftime('%y%m%d%H%M%S')
        ...
        output = payload(
            ...
            longitude="0.000000", 
            utc=utc, 
            status="V",  # V = Invalid (Trigger para DEFCON 1 Pass)
            sats="0",    # Sin satélites
            gsmStrenght="12", # Buena señal GSM (para sospecha de jammer)
            speed="0", 
            direction="0", 
            accuracy="0", 
            altitude="0", 
