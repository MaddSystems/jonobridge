...
cd "${REPO_ROOT}/pkg/integrators/dicsa"
go build

# Clean up any existing Docker images with the dicsatrack name
echo "Removing old Docker images..."
docker images --filter=reference="*dicsa*" --format "{{.ID}}" | xargs -r docker rmi -f
...
docker build -t dicsa -f ./pkg/integrators/dicsa/Dockerfile .

# Tag and push the image
echo "Tagging Docker image..."
docker tag dicsa maddsystems/dicsa:1.0.0
echo "Pushing Docker image..."
docker push maddsystems/dicsa:1.0.0
