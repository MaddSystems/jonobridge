...
import (
	...
	"os"
	"time"
	uscentralintegrator "uscentral/features/uscentral_integrator"
...
func main() {
	...
	// Configure and initialize uscentral before anything else
	uscentralToken := os.Getenv("USCENTRAL_TOKEN")
	uscentralURL := os.Getenv("USCENTRAL_URL")
	fmt.Println("token length:", len(uscentralToken))
	fmt.Println("url:", uscentralURL)

	// Initialize the integrator explicitly
	uscentralintegrator.Initialize()
	...
	clientID := fmt.Sprintf("USCENTRAL_%s_%s_%d",
		"USCENTRAL",
		os.Getenv("HOSTNAME"),
	...
	opts.SetDefaultPublishHandler(func(client mqtt.Client, msg mqtt.Message) {
		...
		uscentralintegrator.Init(string(msg.Payload()))
