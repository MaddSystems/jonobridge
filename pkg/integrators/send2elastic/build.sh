...
cd "${REPO_ROOT}/pkg/integrators/send2elastic"
go build

# Clean up any existing Docker images with the send2elastictrack name
echo "Removing old Docker images..."
docker images --filter=reference="*send2elastic*" --format "{{.ID}}" | xargs -r docker rmi -f
...
docker build -t send2elastic -f ./pkg/integrators/send2elastic/Dockerfile .
