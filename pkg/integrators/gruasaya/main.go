...
import (
	gruasayaintegrator "gruasaya/features/gruasaya_integrator"
	"flag"
	"fmt"
	"log"
	"os"
...
func main() {
	...
	// Configure and initialize gruasaya before anything else
	gruasayaToken := os.Getenv("GRUASAYA_TOKEN")
	gruasayaURL := os.Getenv("GRUASAYA_URL")
	fmt.Println("token length:", len(gruasayaToken))
	fmt.Println("url:", gruasayaURL)

	// Initialize the integrator explicitly
	gruasayaintegrator.Initialize()
	...
	clientID := fmt.Sprintf("GRUASAYA_%s_%s_%d",
		"GRUASAYA",
		os.Getenv("HOSTNAME"),
	...
	opts.SetDefaultPublishHandler(func(client mqtt.Client, msg mqtt.Message) {
		...
		gruasayaintegrator.Init(string(msg.Payload()))
