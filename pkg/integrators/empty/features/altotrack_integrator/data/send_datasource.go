...
var altotrackToken string
var altotrackURL string

// Initialize function to be called once at startup
func InitAltotrack() {
	altotrackToken = os.Getenv("ALTOTRACK_TOKEN")
	if altotrackToken == "" {
		altotrackToken = "87a0239529b1515b2a7a5a173699e3310a5404e1d8b2c8400a6139e4" // Default fallback
	}

	altotrackURL = os.Getenv("ALTOTRACK_URL")
	if altotrackURL == "" {
		altotrackURL = "https://pegasus248.peginstances.com/receivers/json" // Default fallback
	}

	utils.VPrint("Initialized Altotrack with URL: %s and token length: %d", altotrackURL, len(altotrackToken))
}

func ProcessAndSendAltotrack(plates, eco, vin, dataStr string) error {
