...
var (
	version   string = "unknown" // Valor predeterminado
...
func main() {
	...
	utils.VPrint("MQTT_BROKER_HOST: %s", mqttBrokerHost)
	brokerURL := fmt.Sprintf("tcp://%s:1883", mqttBrokerHost)
	opts.AddBroker(brokerURL)
	clientID := fmt.Sprintf("recursoconfiable_%s_%s_%d",
		subscribe_topic,
		os.Getenv("HOSTNAME"),
	...
	opts.SetDefaultPublishHandler(func(client mqtt.Client, msg mqtt.Message) {
		...
		// Send jonoprotocol to process for recursoconfiable
		trackerData := string(msg.Payload())
		recursoconfiable_integrator.Initialize(trackerData)
	})
