...
cd "${REPO_ROOT}/pkg/integrators/recursoconfiable"
go build

# Clean up any existing Docker images with the recursoconfiable name
echo "Removing old Docker images..."
docker images --filter=reference="*recursoconfiable*" --format "{{.ID}}" | xargs -r docker rmi -f
...
docker build -t recursoconfiable -f ./pkg/integrators/recursoconfiable/Dockerfile .
